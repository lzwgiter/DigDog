...
from DigDogConfig import SAVE_PATH
...
class MemoryDumpGenerator(object):
    ...
    def compress(self, input_file, output_file):
        logging.info("正在压缩%r至%r", input_file, output_file)
        with open(input_file, 'rb') as f_in:
            f_out = GzipFile(output_file, 'wb', compresslevel=1)
            while True:
    ...
    def __start_virtual_box(self):
        ...
        self.vb.reset()
        self.vb.start()
        logging.info("请等待%r秒来保证虚拟机启动", self.settings['startDelay'])
...
def generate_dump(sample, output_file, vm_settings, compress, overwrite, notify):
    logging.info("内存输出文件: %r", output_file)
