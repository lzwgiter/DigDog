...
class VirtualBox:
    ...
    def delete_image(medium):
        logging.info("删除iso %r中...", medium.name)
    ...
    def copy_to_guest(self, src, dest):
        ...
        """
        self.__check()
        logging.info("将'%s'拷贝至虚拟机'%s'中" % (src, dest))
        guestSession = self.__get_guest_session()
    ...
    def copy_from_guest(self, src, dest):
        ...
        """
        self.__check()
        logging.info("拷贝虚拟机'%s'至本地'%s'中" % (src, dest))
        guestSession = self.__get_guest_session()
    ...
    def copy_folder_to_guest(self, src, dest):
        ...
        """
        self.__check()
        logging.info("拷贝文件夹'%s'至虚拟机'%s'中" % (src, dest))
    ...
    def execute(self, path, args=None, wait=False):
        ...
        if not args:
            args = list()
        self.__check()
        logging.info("执行 %s %s" % (os.path.basename(path), ' '.join(args)))
        guestSession = self.__get_guest_session()
